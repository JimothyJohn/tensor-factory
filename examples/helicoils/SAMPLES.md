...
Prompts for `tensor-factory-synth --backend gemini` (Nano Banana / `gemini-2.5-flash-image`),
...
from tensor_factory_synth.generator import NanoBananaGenerator
...
The batch tool is `packages/tensor-factory-synth/scripts/gen_samples.py` (this whole catalog
lives there as the single source of truth):

```bash
# Text-only, 25 samples into images/ (clears the old batch first):
uv run --with google-genai python packages/tensor-factory-synth/scripts/gen_samples.py
