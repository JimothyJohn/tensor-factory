"""FastMCP server exposing the helicoil detector as MCP tools (stdio transport).

Tools are thin wrappers over :mod:`helicoils_mcp.core`; all are read-only, local, and
return JSON strings. Run with ``helicoils-mcp`` (stdio) or register in an MCP client.
"""
...
@mcp.tool(name="helicoils_detect", annotations=_read_only("Detect Helicoil"))
def helicoils_detect(params: DetectInput) -> str:
    """Detect the helicoil in a single image and return its bounding box.
...
@mcp.tool(name="helicoils_model_info", annotations=_read_only("Helicoil Model Info"))
def helicoils_model_info(params: _ModelParams) -> str:
...
@mcp.tool(
    name="helicoils_benchmark",
    annotations=_read_only("Benchmark Helicoil Model"),
)
def helicoils_benchmark(params: BenchmarkInput) -> str:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="helicoils-mcp", description="MCP server for helicoil detection (stdio)."
    )
